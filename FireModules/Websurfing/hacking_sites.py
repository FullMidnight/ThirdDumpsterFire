...
# Version: 1.0.0
#
# Author: Author:  Emmanuel Ferran (FullMidnight) based off the work by Joe Gervais (TryCatchHCF)
#
# Summary:
#
...
class hacking_sites( FireModule ):
	...
	def Ignite( self ):
		...
		self.webSession = urllib.request.urlopen( 'https://www.exploit-db.com/' )
		trash = self.webSession.read()

		self.SleepSession()

		print (self.commentsStr + ": Opening URL session to ExploitDB exploits")
		self.webSession = urllib.request.urlopen( 'https://www.exploit-db.com/browse/' )
		trash = self.webSession.read()

		self.SleepSession()

		print (self.commentsStr + ": Opening URL session to ExploitDB Google hacking DB")
		self.webSession = urllib.request.urlopen( 'https://www.exploit-db.com/google-hacking-database/' )
		trash = self.webSession.read()

		self.SleepSession()

		print (self.commentsStr + ": Opening URL session to SecList FullDisclosure site")
		self.webSession = urllib.request.urlopen( 'http://seclists.org/fulldisclosure/' )
		trash = self.webSession.read()

		self.SleepSession()

		print (self.commentsStr + ": Opening URL session to Shodan (systems with default passwords)")
		self.webSession = urllib.request.urlopen( 'https://www.shodan.io/search?query=%22default+password%22' )
		trash = self.webSession.read()
