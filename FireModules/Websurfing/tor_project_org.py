...
class tor_project_org( FireModule ):
	...
	def Ignite( self ):
		...
		self.webSession = urllib.request.urlopen( 'https://www.torproject.org/' )
		trash = self.webSession.read()

		self.SleepSession()

		print (self.commentsStr + ": Opening URL session to Tor Project blog page")
		self.webSession = urllib.request.urlopen( 'https://blog.torproject.org/' )
		trash = self.webSession.read()

		self.SleepSession()

		print (self.commentsStr + ": Opening URL session to Tor Browser page")
		self.webSession = urllib.request.urlopen( 'https://www.torproject.org/projects/torbrowser.html' )
		trash = self.webSession.read()
