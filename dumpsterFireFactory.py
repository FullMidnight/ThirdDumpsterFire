...
# Version: 1.0.1
#
# Author:   Emmanuel Ferran (FullMidnight) based on the work of Joe Gervais (TryCatchHCF)
#
# Summary:  
#
...
def BuildDumpsterFire():
	...
	print ("")
	choice = input("Continue building this Dumpster Fire? [y/n]: ")
	...
	if choice == "y":
		...
		print ("" )
		print ("" )
		choice = input("Save this Dumpster Fire? [y/n]: ")
...
def ConfigureDumpsterFire( thisDumpsterFire ):
	...
	print ("")
	thisDumpsterFire.mName = input( "Enter name for new Dumpster Fire: " )
	print ("")
	...
	print ("")
	choice = input( "Create new time-delay for dumpster fire? [y/n]: ")
...
def AddFires( thisDumpsterFire ):
	...
	while not done:
		...
		choice = input("Add another Fire? [y/n]: ")
	...
	print ("")
	choice = input("Add time offsets to each Fire? [y/n]: ")
	print ("")
...
def AddTimeOffsetsToFires( thisDumpsterFire ):
	...
	while ( i < len( thisDumpsterFire.mFires )):
		...
		while ( done == 0 ):
			...
			keepTimeOffset = input( "Keep time offset? (y/n): " )
...
def BuildTimeOffsetForFire( thisFire ):
	...
	print ("")
	print ("Time offset for Fire '" + thisFire.mFireName + "': " + str( hours ).zfill( 2 ) + "hours," +  str( minutes ).zfill( 2 ) + "minutes")
	print ("")
...
def SaveDumpsterFire( newDumpsterFire ):
	...
	print ("")
	newDumpsterFire.mDescription = input( "Enter description of new Dumpster Fire: " )
...
def PrintDumpsterFireConfig( thisDumpsterFire ):
	...
	while ( i < len( thisDumpsterFire.mFires )):
		print ("\t** ", thisDumpsterFire.mFires[ i ].mFireName)
		print ("\t    Relative Time Delay (HH:MM): " + str( thisDumpsterFire.mFires[ i ].mOffsetHours ).zfill(2) + ":" +  str( thisDumpsterFire.mFires[ i ].mOffsetMinutes ).zfill(2))
...
def IgniteDumpsterFire( thisDumpsterFire, withUserInteraction ):
	...
	while ( i < len( thisDumpsterFire.mFires )):
		...
		if ( currentFire.mOffsetHours > 0 or currentFire.mOffsetMinutes > 0 ):
			...
			print ("Delayed Fire ignition detected. Pausing for (HH:MM)" +str( currentFire.mOffsetHours ).zfill(2) + ":" + str( currentFire.mOffsetMinutes ).zfill(2))
			print ("(Waiting...)")
...
def StartDumpsterFire():
	...
	choice = input( "Ignite Dumpster Fire? [y/n]: ")
...
def SelectFireCategory():
	...
	while ( selection < 0 or selection > (fireCatCount - 2)):
		try:
			fireCategoryNum = input( "Enter Fire Category#: " )
...
def BuildDateTime():
	...
	while ( done == 0 ):
				...
				if ( year < fireStartTime.year ):
					...
					choice = input("You're about to hack time, are you sure? [y/n]: ")	
...
def SelectDumpsterFire():
	...
	while ( selection < 0 or selection > ( dumpsterfireCount - 2 )):
		try:
			dumpsterfireNum = input( "Enter DumpsterFire #: " )
...
def DeleteDumpsterFire():
	...
	if ( selection > -1 ):
		...
		print ("")
		choice = input( "Delete this DumpsterFire? (y/n): " )
